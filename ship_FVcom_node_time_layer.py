...
'''
wrong version
obsData=pd.read_csv('shipdata.csv')       #this is ship data
...
'''    
obsData=pd.read_csv('shipdata.csv',index_col=0)
good_index=np.where(obsData.index!=2564)[0]
#shipid=obsData['id'][good_index]
...
obsTime=pd.Series((datetime.strptime(x, "%Y-%m-%d %H:%M:%S") for x in obsData['time'][good_index]),index=good_index)
